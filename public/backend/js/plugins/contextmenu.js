...
class ContextMenu {
  ...
  _initIcons() {
    jQuery.contextMenu({
      selector: '#sendingListMenu',
      animation: {duration: 0},
        callback: function (key, options) {
            let id = $(this).data('id');
            switch (key) {
                case 'copy':
                    window.location.href = '/admin/sending/copy/' + id;
                    break;
                case 'archive':
                    window.location.href = '/admin/sending/archive/' + id;
                    break;
                case 'delete':
                    if (confirm('Silinsin?')) {
                        window.location.href = '/admin/sending/delete/' + id;
                    }
                    break;
            }
        },
